...
def page_report():
    ...
    st.markdown("---")
    st.markdown("### 🗺️ AI Response Surface Plot")
    st.markdown("Visualize the 3D experimental landscape generated by the AI, exactly like a classical RSM plot.")
    
    if len(st.session_state.factor_cols) >= 2:
        import plotly.graph_objects as go
        import numpy as np
        
        c1, c2 = st.columns(2)
        with c1:
            x_axis = st.selectbox("X-axis Factor", st.session_state.factor_cols, index=0)
        with c2:
        ...
        if st.button("Generate Plot", type="primary"):
            with st.spinner(f"Rendering AI {plot_type}..."):
                try:
                    ...
                    x_min, x_max = X_orig_df[x_axis].min(), X_orig_df[x_axis].max()
                    y_min, y_max = X_orig_df[y_axis].min(), X_orig_df[y_axis].max()
                    
                    x_grid = np.linspace(x_min, x_max, 40)
                    y_grid = np.linspace(y_min, y_max, 40)
                    X_mesh, Y_mesh = np.meshgrid(x_grid, y_grid)
                    
                    # Create base DataFrame using optimums
                    base_dict = st.session_state.ai_optimum.copy()
                    
                    # Flatten mesh for prediction
                    grid_df = pd.DataFrame([base_dict] * (40*40))
                    grid_df[x_axis] = X_mesh.flatten()
                    grid_df[y_axis] = Y_mesh.flatten()
